...
namespace Codeplex.Data
{
    public class DynamicJson : DynamicObject
    {
        ...
        #region 扩展XML
        public static dynamic XmlToJson(string value)
        {
            XElement xmlDoc = XElement.Parse(value);
            string result = CreateJsonString(new XStreamingElement("root", CreateTypeAttr(JsonType.@object), xmlDoc.Elements()));
            return Parse(result);
        }

        public static string JsonToXml(string value)
        {
            DynamicJson json = Parse(value);
            return json.xml.ToString();
        }
        #endregion
